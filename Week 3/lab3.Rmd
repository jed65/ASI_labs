...
We should expect that inv_avg_hessian and inv_var_cov_grad should be close to each other since they are both equal to $\mathcal{I}^{-1}$. But are values are not close to each other and are out bu orders of magnitude. This means that something has gone wrong somewhere but we can't find the bug.
...
We can use the inverse Fisher information matrix calculated above to generate a normally distributed variable $\hat{\mu}\sim N(\mu^*,\mathcal{I}_{22}^{-1})$ , since $\theta_2^*$ is the mean number of deaths due to COVID. This can then be used to generate confidence intervals as shown in the lecture notes in Example 3.5.1.
